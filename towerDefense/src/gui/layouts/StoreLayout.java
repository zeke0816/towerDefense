...
public class StoreLayout extends Layout<GridPane> {
	...
	public void updateAvailability() {
		// TODO: check budget, not inventory
		layout.getChildren().clear();
		Game game = Game.getInstance();
		
		layout.add(bb8.getDisabledLabel(), 0, 0);
		if(game.canAfford(bb8.getWarrior().getPrice())) {
		...
		if(game.canAfford(flea.getWarrior().getPrice())) {
		...
		if(game.canAfford(gary.getWarrior().getPrice())) {
		...
		if(game.canAfford(turret.getWarrior().getPrice())) {
		...
		if(game.canAfford(agentP.getWarrior().getPrice())) {
		...
		if(game.canAfford(cyborg.getWarrior().getPrice())) {
		...
		if(game.canAfford(shield.getItem().getPrice())) {
		...
		if(game.canAfford(cure.getItem().getPrice())) {
		...
		if(game.canAfford(poison.getItem().getPrice())) {
		...
		if(game.canAfford(barricade.getItem().getPrice())) {
		...
		if(game.canAfford(nuke.getItem().getPrice())) {
